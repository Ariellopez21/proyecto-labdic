...
En primer lugar, Alembic permite manejar de forma estructurada la evolución del esquema de la base de datos, lo que resulta esencial en proyectos donde el modelo de datos se encuentra en constante ajuste durante las fases de desarrollo ~\cite{pingcap_alembic}. Esta capacidad evita inconsistencias entre los entornos de desarrollo y prueba, y sienta las condiciones necesarias para una futura transición a un entorno de producción sin contratiempos.
...
Un aspecto clave para la elección de Docker es su capacidad de asegurar la reproducibilidad del entorno. Gracias a su funcionamiento basado en contenedores, es posible ejecutar la misma configuración de manera idéntica en distintos sistemas~\cite{microsoft_microservices_orchestration}, lo que resulta especialmente relevante de cara a un eventual despliegue del sistema en los servidores del laboratorio. Este aspecto, que escapa a los límites del presente proyecto, queda respaldado por la elección de Docker Compose como orquestador: el sistema está preparado para ser desplegado en producción en el momento en que se decida avanzar en esa dirección. Así, los problemas recurrentes de reinstalación manual, inconsistencias entre entornos y pérdida de tiempo quedan resueltos de antemano, garantizando un entorno consistente, confiable y fácil de replicar~\cite{tildavps_docker}.