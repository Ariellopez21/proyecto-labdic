...
\section{Diseño del modelo de base de datos}

\subsection{Grandes grupos funcionales}\label{sec:grandes_grupos}

En una primera aproximación, el sistema puede describirse en torno a tres grandes 
ejes funcionales: los \textbf{usuarios} interactúan con el sistema para solicitar 
un \textbf{préstamo} de \textbf{productos}, y dichas solicitudes son gestionadas 
por los administradores hasta concretar la entrega y registrar la devolución.

Sin embargo, a medida que el diseño avanza, la complejidad de las interacciones 
entre entidades evidencia la necesidad de subdividir estos grandes ejes en módulos 
más pequeños y cohesivos. Este enfoque de \textit{dividir para vencer} permite que 
cada módulo tenga una responsabilidad bien definida, reduzca el acoplamiento entre 
tablas y facilite tanto el mantenimiento como la extensión futura del sistema.

Así, el modelo de base de datos final se organiza en seis módulos funcionales, 
cuyas entidades y relaciones se describen en el cuadro~\ref{tab:grandes_grupos_funcionales}.

\begin{table}[H]
	\centering
	\begin{tabular}{p{0.17\linewidth}p{0.34\linewidth}p{0.37\linewidth}}
		\hline
		\textbf{Módulo} & \textbf{Propósito principal} & \textbf{Entidades representativas} \\
		\hline
		Usuarios & Gestionar la identidad, credenciales y datos personales de quienes 
		interactúan con el sistema.
		& \texttt{users}, \texttt{user\_roles} \\
		\hline
		Autenticación & Controlar el acceso mediante credenciales cifradas y permisos 
		por rol, sin requerir tablas adicionales: se implementa a través de campos 
		específicos del modelo de usuario.
		& Campos \texttt{password}, \texttt{is\_active}, \texttt{is\_admin} en \texttt{users} \\
		\hline
		Catálogo & Centralizar los datos de referencia reutilizables por otros módulos 
		del sistema, evitando redundancia y facilitando la administración. Incluye 
		\texttt{roles}, que inicialmente pertenecía al módulo de usuarios pero fue 
		reubicado aquí por compartir la misma naturaleza de dato de referencia con 
		funcionalidades CRUD simples y acotadas.
		& \texttt{roles}, \texttt{brands}, \texttt{models}, \texttt{categories}, 
		\texttt{statuses}, \texttt{ubications} \\
		\hline
		Productos & Representar los tipos de ítems disponibles para préstamo y sus 
		características descriptivas.
		& \texttt{products} \\
		\hline
		Dispositivos & Administrar las unidades físicas individuales del inventario, 
		su estado actual y el historial de cambios de estado.
		& \texttt{devices}, \texttt{device\_status\_logs} \\
		\hline
		Préstamos & Modelar el ciclo de vida completo de las solicitudes de préstamo, 
		desde la solicitud inicial hasta la devolución.
		& \texttt{loan\_requests}, \texttt{loan\_request\_items} \\
		\hline
	\end{tabular}
	\caption{Módulos funcionales del modelo de datos de LabDIC.}
	\label{tab:grandes_grupos_funcionales}
\end{table}

Cabe destacar que la tabla \texttt{statuses} es transversal al modelo: sus registros 
son referenciados tanto por \texttt{devices} como por \texttt{loan\_requests} y 
\texttt{device\_status\_logs}, lo que refleja que el concepto de \textit{estado} 
opera como un eje compartido entre los módulos de dispositivos y préstamos.

\subsection{División en tablas modulares}\label{sec:div_tablas_modulares}

Cada módulo funcional se implementa mediante una o más tablas, diseñadas para 
ser cohesivas internamente y débilmente acopladas entre sí. Las relaciones entre 
módulos se establecen a través de claves foráneas, preservando la integridad 
referencial del modelo. Cualquier cambio estructural futuro en el esquema se 
gestiona mediante migraciones controladas con Alembic, garantizando trazabilidad 
y consistencia a lo largo del ciclo de vida del proyecto.
...
La tabla \texttt{user\_roles} actúa como tabla intermedia que resuelve la relación 
muchos a muchos entre usuarios y roles. Un usuario puede tener múltiples roles 
asignados y un mismo rol puede estar asociado a múltiples usuarios. Su estructura 
es deliberadamente minimalista, limitándose a las claves foráneas necesarias para 
...
\begin{table}[H]
	...
	\begin{tabular}{|p{0.25\linewidth}|p{0.7\linewidth}|}
		...
		\textbf{Campo} & \textbf{Descripción} \\
		\hline
		\texttt{user\_id} & Clave foránea que referencia a \texttt{users.id}. 
		Forma parte de la clave primaria compuesta. \\
		\hline
		\texttt{role\_id} & Clave foránea que referencia a \texttt{roles.id}. 
		Forma parte de la clave primaria compuesta. \\
		\hline
	\end{tabular}
	\caption{Estructura de la tabla intermedia \texttt{user\_roles}.}
	\label{tab:tabla_user_roles}
\end{table}

\subsubsection{Módulo de Autenticación}

El módulo de autenticación no requiere tablas propias en el esquema relacional. 
Su implementación se sustenta en campos específicos de la tabla \texttt{users}: 
\texttt{password} almacena la contraseña cifrada mediante hashing, mientras que 
\texttt{is\_active} e \texttt{is\_admin} controlan el estado de la cuenta y el 
nivel de acceso del usuario respectivamente. Complementariamente, la relación 
muchos a muchos con \texttt{roles} a través de \texttt{user\_roles} permite 
implementar un control de acceso granular basado en roles.

Este módulo ilustra que la modularidad del sistema no se define exclusivamente 
por la cantidad de tablas que lo componen, sino por la responsabilidad funcional 
que agrupa: en este caso, garantizar que solo usuarios válidos y autorizados 
puedan operar el sistema.

\subsubsection{Módulo de Catálogo}
...
\subsubsection{Módulo de Dispositivos}
...
\subsubsection{Módulo de Prestamos}\label{sec:modulo_prestamos}
...
\subsection{Modelo de base de datos}\label{sec:modelo_DB}
...
\subsubsection{Diagrama Entidad Relación}\label{sec:diagrama_ER}
...
\begin{figure}[H]
	...
	\resizebox{\textwidth}{0.80\textheight}{%
		\begin{tikzpicture}[node distance=0.7cm and 1.0cm]
			...
			% ── Catálogo fila 1 ──────────────────────────────────────────────────────────
			\node[ent] (brand) at (0,0) {
				\nodepart{one}\textbf{\color{white}Brand}
				\nodepart{two}\underline{id}: INT PK\\ name: VARCHAR(100)
			};
			\node[ent, right=1.2cm of brand] (model) {
				\nodepart{one}\textbf{\color{white}Model}
				\nodepart{two}\underline{id}: INT PK\\ name: VARCHAR(100)
			};
			\node[ent, right=1.2cm of model] (category) {
				\nodepart{one}\textbf{\color{white}Category}
				\nodepart{two}\underline{id}: INT PK\\ name: VARCHAR(100)
			};
			\node[ent, right=1.2cm of category] (ubication) {
				\nodepart{one}\textbf{\color{white}Ubication}
				\nodepart{two}\underline{id}: INT PK\\ name: VARCHAR(100)\\ description: VARCHAR(500)
			};
			\node[ent, right=1.2cm of ubication] (status) {
				\nodepart{one}\textbf{\color{white}Status}
				\nodepart{two}\underline{id}: INT PK\\ name: VARCHAR(50)
			};
			
			% ── Product fila 2 ───────────────────────────────────────────────────────────
			\node[ent, below=1.4cm of model] (product) {
				\nodepart{one}\textbf{\color{white}Product}
				\nodepart{two}
				\underline{id}: INT PK\\ name: VARCHAR(100)\\ brand\_id: INT FK\\ model\_id: INT FK\\ category\_id: INT FK\\ description: VARCHAR(500)\\ is\_active: BOOL\\ created\_at: DATETIME
			};
			
			% ── Device fila 3 ────────────────────────────────────────────────────────────
			\node[ent, below=1.4cm of product] (device) {
				\nodepart{one}\textbf{\color{white}Device}
				\nodepart{two}
				\underline{id}: INT PK\\ product\_id: INT FK\\ internal\_code: VARCHAR(50)\\ serial\_number: VARCHAR(50)\\ status\_id: INT FK\\ ubication\_id: INT FK\\ created\_at: DATETIME
			};
			
			% ── LoanRequestItem + DeviceStatusLog fila 4 ─────────────────────────────────
			\node[ent, below=1.4cm of device] (lri) {
				\nodepart{one}\textbf{\color{white}LoanRequestItem}
				\nodepart{two}
				\underline{id}: INT PK\\ loan\_request\_id: INT FK\\ device\_id: INT FK
			};
			\node[ent, right=1.2cm of lri] (dsl) {
				\nodepart{one}\textbf{\color{white}DeviceStatusLog}
				\nodepart{two}
				\underline{id}: INT PK\\ device\_id: INT FK\\ status\_id: INT FK\\ user\_id: INT FK\\ timestamp: DATETIME
			};
			
			% ── LoanRequest fila 5 ────────────────────────────────────────────────────────
			\node[ent, below=1.4cm of lri] (loanreq) {
				\nodepart{one}\textbf{\color{white}LoanRequest}
				\nodepart{two}
				\underline{id}: INT PK\\ user\_id: INT FK\\ status\_id: INT FK\\ reason: VARCHAR(500)\\ request\_date: DATETIME\\ delivery\_date: DATETIME\\ estimated\_return: DATETIME\\ actual\_return: DATETIME
			};
			
			% ── User / UserRole / Role columna izquierda ──────────────────────────────────
			\node[ent, left=1.8cm of loanreq] (user) {
				\nodepart{one}\textbf{\color{white}User}
				\nodepart{two}
				\underline{id}: INT PK\\ rut: VARCHAR(12)\\ name: VARCHAR(100)\\ username: VARCHAR(50)\\ email: VARCHAR(100)\\ password: VARCHAR(256)\\ phone: VARCHAR(18)\\ address: VARCHAR(360)\\ is\_active: BOOL\\ is\_admin: BOOL\\ created\_at: DATETIME
			};
			\node[piv, above=1.0cm of user] (userrole) {
				\nodepart{one}\textbf{\color{white}UserRole}
				\nodepart{two}user\_id: INT FK (PK)\\ role\_id: INT FK (PK)
			};
			\node[ent, above=1.0cm of userrole] (role) {
				\nodepart{one}\textbf{\color{white}Role}
				\nodepart{two}\underline{id}: INT PK\\ name: VARCHAR(50)\\ description: VARCHAR(255)
			};
			
			% ── Relaciones ────────────────────────────────────────────────────────────────
			\draw[rel] (brand.south) -- ++(0,-0.1) -| (product.north west)
			node[pos=0.2, above, font=\tiny\color{relfg}]{1};
			\draw[rel] (model.south) -- (product.north)
			node[midway, right, font=\tiny\color{relfg}]{1};
			\draw[rel] (category.south) -- ++(0,-0.1) -| (product.north east)
			node[pos=0.2, above, font=\tiny\color{relfg}]{1};
			\draw[rel] (product.south) -- (device.north)
			node[midway, right, font=\tiny\color{relfg}]{1..N};
			\draw[rel] (ubication.south) -- ++(0,-0.15) -| (device.east)
			node[pos=0.15, right, font=\tiny\color{relfg}]{1};
			\draw[rel] (status.south) -- ++(0,-0.1) -- ++(0.5,0) |- (device.east)
			node[pos=0.1, right, font=\tiny\color{relfg}]{1};
			\draw[rel] (status.south) -- ++(0,-0.1) -- ++(0.5,0) |- (loanreq.east)
			node[pos=0.85, right, font=\tiny\color{relfg}]{1};
			\draw[rel] (status.south) -- ++(0,-0.1) -- ++(0.15,0) |- (dsl.east)
			node[pos=0.5, right, font=\tiny\color{relfg}]{1};
			\draw[rel] (device.south) -- (lri.north)
			node[midway, right, font=\tiny\color{relfg}]{1..N};
			\draw[rel] (device.east) -| (dsl.north)
			node[pos=0.25, above, font=\tiny\color{relfg}]{1..N};
			\draw[rel] (loanreq.north) -- (lri.south)
			node[midway, right, font=\tiny\color{relfg}]{1..N};
			\draw[rel] (user.east) -- (loanreq.west)
			node[midway, above, font=\tiny\color{relfg}]{1..N};
			\draw[rel] (user.north) -- ++(0,0.2) -- ++(4.8,0) |- (dsl.west)
			node[pos=0.8, above, font=\tiny\color{relfg}]{1..N};
			\draw[rel] (user.north) -- (userrole.south)
			node[midway, right, font=\tiny\color{relfg}]{N};
			\draw[rel] (role.south) -- (userrole.north)
			node[midway, right, font=\tiny\color{relfg}]{N};
			
		\end{tikzpicture}
	}% fin resizebox
	\caption{Diagrama Entidad-Relación del modelo de base de datos LabDIC.}
	\label{fig:diagrama_modelo_entidad_relacion}
\end{figure}