...
ningún elemento de navegación. \texttt{AppLayout} es el layout principal para todas 
...
contrario, a \texttt{HomeView}. Los errores de credenciales (\texttt{401}) y de 
...
sus roles mediante el repositorio de \texttt{roles}. \texttt{MyUserView} expone los 
datos del usuario autenticado obtenidos desde \texttt{GET /users/me}, permitiendo 
consultar su perfil sin necesidad de conocer su identificador numérico. 
\texttt{AdminRolesView} es la vista de administración de roles, accesible en 
\texttt{/admin/roles}.
...
\texttt{statuses} y \texttt{ubications}. Cada entidad se presenta en una sección 
...
Una consideración importante respecto a \texttt{statuses}: los estados del sistema 
...
ubicación del catálogo. Al igual que en la fase anterior, la eliminación de un 
...
según lo descrito en \ref{sec:alembic_uso}.
...
verificación. Al confirmar la selección, se abre un diálogo donde el usuario puede 
ingresar un motivo y una fecha estimada de devolución, dejando todo listo para enviar 
la solicitud de préstamo. La lógica de creación del préstamo, sin embargo, no se 
...
está aprobada, y registrar devolución si está prestada.
...
El layout basado en \texttt{AppSidebar} colapsable y el uso de componentes de PrimeVue 
con soporte nativo para pantallas pequeñas permitió que la interfaz se adapte de forma 
satisfactoria tanto a resoluciones de escritorio como a dispositivos móviles en 
